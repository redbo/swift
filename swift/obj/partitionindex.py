...
from swift.common.utils import write_pickle, remove_file as unlink, \
    lock_path, lock_file, preload_buffer_cache


JOURNAL_FILE = 'hashes.db.pending'
...
class PartitionIndexDB(sqlite3.Connection):
    """
    Wraps all operations on the partition hashes database behind methods.
    ...
    def __init__(self, path, *args, **kwargs):
        ...
        with lock_path(database_dir):
            ...
            super(PartitionIndexDB, self).__init__(path, *args, **kwargs)
    ...
    def add_remove_files(self, adds, removes):
        """
        Add and remove files from the database.
        """
        ...
        for filename in adds:
            ...
            md5a, md5b = struct.unpack('!qq', hashlib.md5(filename).digest())
            if ext == 'ts':
                add_args.append((filename, suffix, timestamp, md5a, md5b))
            else:
                add_args.append((filename, suffix, None, md5a, md5b))
        for filename in removes:
            suffix, obj_hash, timestamp, ext = file_info(filename)
            modified_suffixes.add(suffix)
            remove_args.append((filename,))
        self.executemany("INSERT OR IGNORE INTO filelist (filename, suffix, "
                         "tombstone_ts, md5a, md5b) VALUES (?, ?, ?, ?, ?)",
                         add_args)
    ...
    def mark_populated(self, suffix):
        """
        Remove the repopulate flag from a suffix.
        """
        self.execute('UPDATE hashes SET repopulate = 0 WHERE suffix=?',
                     (suffix,))

    def get_hash(self, suffix):
        """
        Get the hash for the given suffix.
        """
    ...
    def expired_tombstones(self, reclaim_age):
        """
        Fetch a list of tombstones older than reclaim_age.
        """
    ...
    def get_hashes(self):
        """
        Get a dictionary of all suffix hashes.
        """
        hashes = {}
    ...
    def files_in_suffixes(self, suffixes):
        """
        Get a list of all files in the given suffixes.
        """
        hashlist = []
        for rec in self.execute('SELECT filename FROM filelist WHERE suffix in'
                                ' (%s)' % ','.join('?' * len(suffixes)),
                                suffixes):
...
def _open_partition_db(partition_dir):
    ...
    with lock_file(os.path.join(partition_dir, JOURNAL_FILE)) as journal_file:
        ...
        if adds or removes:
            db.add_remove_files(adds, removes)
            db.commit()
    try:
        yield db
    except Exception:
