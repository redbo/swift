...
func (r *Replicator) listObjFiles(objChan chan string, done chan struct{}, partdir string, needSuffix func(string) bool) {
	defer close(objChan)
...
func (r *Replicator) replicateLocal(j *job, nodes []*hummingbird.Device, moreNodes hummingbird.MoreNodes) {
	...
	defer close(done)
	go r.listObjFiles(objChan, done, path, func(suffix string) bool {
		for _, remoteHash := range remoteHashes {
...
func (r *Replicator) replicateHandoff(j *job, nodes []*hummingbird.Device) {
	...
	defer close(done)
	go r.listObjFiles(objChan, done, path, func(string) bool { return true })
	for objFile := range objChan {
		toSync := make([]*syncFileArg, 0)
		for _, dev := range nodes {
