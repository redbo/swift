...
func BenchmarkMergeItems(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		for i := 0; i < 10000; i++ {
			recs[i] = &ObjectRecord{Name: hummingbird.UUID(), CreatedAt: "20000000.00001", Deleted: 1}
		}
		db.MergeItems(recs, "")
		db.MergeItems(recs, "")
	}
...
func TestCheckSyncLink(t *testing.T) {
	...
	db.CheckSyncLink()
	require.True(t, hummingbird.Exists(link))
	db.UpdateMetadata(map[string][]string{
		"X-Container-Sync-To": []string{"", "200000001.00001"},
	})
	db.CheckSyncLink()
	require.False(t, hummingbird.Exists(link))
}
