...
type patchableReplicationDevice struct {
	...
	_sendReplicationMessage    func(dev *hummingbird.Device, part uint64, ringHash string, args ...interface{}) (int, []byte, error)
	_sync                      func(dev *hummingbird.Device, part uint64, ringHash string, info *ContainerInfo) (*ContainerInfo, error)
	_rsync                     func(dev *hummingbird.Device, c ReplicableContainer, part uint64, op string) error
	_usync                     func(dev *hummingbird.Device, c ReplicableContainer, part uint64, localID string, point int64) error
	_chooseReplicationStrategy func(localInfo, remoteInfo *ContainerInfo, usyncThreshold int64) string
	_replicateDatabaseToDevice func(dev *hummingbird.Device, c ReplicableContainer, part uint64) error
...
func (d *patchableReplicationDevice) sendReplicationMessage(dev *hummingbird.Device, part uint64, ringHash string, args ...interface{}) (int, []byte, error) {
...
func (d *patchableReplicationDevice) sync(dev *hummingbird.Device, part uint64, ringHash string, info *ContainerInfo) (*ContainerInfo, error) {
	if d._sync != nil {
		return d._sync(dev, part, ringHash, info)
	}
	return d.rd.sync(dev, part, ringHash, info)
}
func (d *patchableReplicationDevice) rsync(dev *hummingbird.Device, c ReplicableContainer, part uint64, op string) error {
	if d._rsync != nil {
		return d._rsync(dev, c, part, op)
	}
	return d.rd.rsync(dev, c, part, op)
}
func (d *patchableReplicationDevice) usync(dev *hummingbird.Device, c ReplicableContainer, part uint64, localID string, point int64) error {
	if d._usync != nil {
...
func (d *patchableReplicationDevice) replicateDatabaseToDevice(dev *hummingbird.Device, c ReplicableContainer, part uint64) error {
...
func newTestReplicationDevice(dev *hummingbird.Device, r *Replicator) *patchableReplicationDevice {
	if r.sendStat == nil {
...
func testServer(t *testing.T, h http.Handler) (*hummingbird.Device, func()) {
...
func TestReplicatorSync(t *testing.T) {
	...
	defer cleanup()
	rd := newTestReplicationDevice(&hummingbird.Device{}, &Replicator{client: http.DefaultClient})
...
func TestReplicatorUsync(t *testing.T) {
	...
	defer cleanup1()
	rd := newTestReplicationDevice(&hummingbird.Device{}, &Replicator{client: http.DefaultClient, perUsync: 1, maxUsyncs: 5})
	c, _, cleanup2, err := createTestDatabase("1410586890.28563")
	require.Nil(t, err)
	require.Nil(t, mergeItemsByName(c, []string{"a", "b", "c"}))
	defer cleanup2()
...
func TestReplicatorReplicateDatabaseToDevice(t *testing.T) {
	...
	require.Nil(t, err)
	defer cleanup()
	rd := newTestReplicationDevice(&hummingbird.Device{}, &Replicator{})
	currentMethod := "complete_rsync"
	rsyncCalled := false
	usyncCalled := false
	rd._sync = func(dev *hummingbird.Device, part uint64, ringHash string, info *ContainerInfo) (*ContainerInfo, error) {
		return &ContainerInfo{}, nil
	}
	rd._chooseReplicationStrategy = func(localInfo, remoteInfo *ContainerInfo, usyncThreshold int64) string {
		return currentMethod
	}
	rd._rsync = func(dev *hummingbird.Device, c ReplicableContainer, part uint64, op string) error {
		rsyncCalled = true
		return nil
	}
	rd._usync = func(dev *hummingbird.Device, c ReplicableContainer, part uint64, localID string, point int64) error {
		usyncCalled = true
		return nil
	}
	require.Nil(t, rd.replicateDatabaseToDevice(&hummingbird.Device{}, c, 1))
...
func TestReplicateDatabase(t *testing.T) {
	...
	rd := newTestReplicationDevice(&hummingbird.Device{}, &Replicator{})
	called := 0
	rd._replicateDatabaseToDevice = func(dev *hummingbird.Device, c ReplicableContainer, part uint64) error {
...
func (r *handoffJobRing) GetJobNodes(partition uint64, localDevice int) (response []*hummingbird.Device, handoff bool) {
	return []*hummingbird.Device{
		&hummingbird.Device{Device: "sda", ReplicationIp: "127.0.0.1", ReplicationPort: 20000},
		&hummingbird.Device{Device: "sdb", ReplicationIp: "127.0.0.2", ReplicationPort: 2000},
	}, true
}
...
func TestReplicateDatabaseDeletesHandoff(t *testing.T) {
	...
	require.Nil(t, err)
	defer cleanup()
	rd := newTestReplicationDevice(&hummingbird.Device{}, &Replicator{Ring: &handoffJobRing{}})
	called := 0
	rd._replicateDatabaseToDevice = func(dev *hummingbird.Device, c ReplicableContainer, part uint64) error {
		require.Equal(t, part, uint64(1))
		if called == 0 {
	...
	rd.replicateDatabase(dbFile)
	require.Equal(t, 2, called)
	require.False(t, hummingbird.Exists(dbFile))
}
...
func TestReplicateDatabaseDeletesHandoffOnSuccess(t *testing.T) {
	...
	rd.replicateDatabase(dbFile)
	require.Equal(t, 2, called)
	require.True(t, hummingbird.Exists(dbFile))
}
...
func TestReplicate(t *testing.T) {
	...
	rd := newTestReplicationDevice(&hummingbird.Device{Device: "sdb"}, rep)
	dbFiles := []string{
		"/path/to/a/db.db",
...
type localDevicesRing struct {
	fakeRing
	localDevs []*hummingbird.Device
}

func (r *localDevicesRing) LocalDevices(localPort int) (devs []*hummingbird.Device, err error) {
	return r.localDevs, nil
}

func TestVerifyDevicesRemoveStuck(t *testing.T) {
	r := &Replicator{
		Ring:   &localDevicesRing{localDevs: []*hummingbird.Device{}},
...
func TestGetReplicator(t *testing.T) {
	...
	GetRing = func(ringType, prefix, suffix string, policy int) (hummingbird.Ring, error) {
		return &fakeRing{}, nil
	}
	conf, err := hummingbird.StringConfig("[container-replicator]\nmount_check=false\nbind_port=1000")
	require.Nil(t, err)
	r, err := GetReplicator(conf, &flag.FlagSet{})
	require.Nil(t, err)
...
func TestReplicateSyncOnMessageError(t *testing.T) {
	rd := newTestReplicationDevice(&hummingbird.Device{}, &Replicator{Ring: &handoffJobRing{}})
	rd._sendReplicationMessage = func(dev *hummingbird.Device, part uint64, ringHash string, args ...interface{}) (int, []byte, error) {
		return 0, nil, errors.New("SOME ERROR")
	}
	rinfo, err := rd.sync(&hummingbird.Device{}, 1, "SOMEHASH", &ContainerInfo{})
	require.Nil(t, rinfo)
	require.NotNil(t, err)
...
func TestSomeDatabaseFailures(t *testing.T) {
	rd := newTestReplicationDevice(&hummingbird.Device{}, &Replicator{Ring: &handoffJobRing{}})
	require.NotNil(t, rd.rsync(&hummingbird.Device{}, fakeDatabase{}, 1, "complete_rsync"))
	require.NotNil(t, rd.usync(&hummingbird.Device{}, fakeDatabase{}, 1, "123", 3))
}
