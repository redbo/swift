...
func makeTestServer() (http.Handler, func(), error) {
	...
	server := &ContainerServer{
		...
		diskInUse:        hummingbird.NewKeyedLimit(2, 2),
		autoCreatePrefix: ".",
	}
	cleanup := func() {
		os.RemoveAll(dir)
	}
	return server.GetHandler(*new(hummingbird.Config)), cleanup, nil
}
...
func makeTestServer2() (*ContainerServer, http.Handler, func(), error) {
	...
	return server, server.GetHandler(*new(hummingbird.Config)), cleanup, nil
}
