...
func (db *sqliteContainer) NewID() error {
	...
	if _, err = tx.Exec("UPDATE container_info SET id = ?", hummingbird.UUID()); err != nil {
		return err
	}
	defer db.invalidateCache()
	return tx.Commit()
...
func (db *sqliteContainer) CleanupTombstones(reclaimAge int64) error {
	...
	if metastr == "" {
		...
		for k, v := range metadata {
			if v[0] == "" {
				if ts, err := hummingbird.GetEpochFromTimestamp(v[1]); err != nil || ts < reclaimTimestamp {
					delete(metadata, k)
					updated = true
				}
			}
...
func (db *sqliteContainer) CheckSyncLink() error {
	...
	} else if hummingbird.Exists(symLoc) {
...
func sqliteCreateContainer(containerFile string, account string, container string, putTimestamp string,
	...
	if hummingbird.Exists(containerFile) {
	...
	if _, err := tx.Exec(`INSERT INTO container_info (account, container, created_at, id, put_timestamp,
		...
		account, container, hummingbird.GetTimestamp(), hummingbird.UUID(), putTimestamp,
