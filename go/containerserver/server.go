...
type ContainerServer struct {
	...
	syncRealms       hummingbird.SyncRealmList
	defaultPolicy    int
}
...
func (server *ContainerServer) ContainerGetHandler(writer http.ResponseWriter, request *http.Request) {
	...
	if err == ErrorNoSuchContainer {
		...
		hummingbird.GetLogger(request).LogError("Unable to get container: %v", err)
	...
	info, err := db.GetInfo()
	if err != nil {
		hummingbird.GetLogger(request).LogError("Unable to get container info: %v", err)
		hummingbird.StandardResponse(writer, http.StatusInternalServerError)
		return
	}
	headers := writer.Header()
	if ts, err := hummingbird.GetEpochFromTimestamp(info.CreatedAt); err == nil {
		headers.Set("X-Backend-Timestamp", ts)
	}
	if ts, err := hummingbird.GetEpochFromTimestamp(info.PutTimestamp); err == nil {
		headers.Set("X-Backend-Put-Timestamp", ts)
	}
	if ts, err := hummingbird.GetEpochFromTimestamp(info.DeleteTimestamp); err == nil {
		headers.Set("X-Backend-Delete-Timestamp", ts)
	}
	if ts, err := hummingbird.GetEpochFromTimestamp(info.StatusChangedAt); err == nil {
	...
	if err != nil {
		hummingbird.GetLogger(request).LogError("Unable to get metadata: %v", err)
		hummingbird.StandardResponse(writer, http.StatusInternalServerError)
		return
	}
	...
	if deleted, err := db.IsDeleted(); err != nil {
		hummingbird.GetLogger(request).LogError("Error calling IsDeleted: %v", err)
		hummingbird.StandardResponse(writer, http.StatusInternalServerError)
		return
	} else if deleted {
		hummingbird.StandardResponse(writer, http.StatusNotFound)
	...
	reverse := hummingbird.LooksTrue(request.Form.Get("reverse"))
	objects, err := db.ListObjects(int(limit), marker, endMarker, prefix, delimiter, path, reverse, policyIndex)
	if err != nil {
		hummingbird.GetLogger(request).LogError("Unable to list objects: %v", err)
		hummingbird.StandardResponse(writer, http.StatusInternalServerError)
		return
	}
...
func (server *ContainerServer) ContainerPutHandler(writer http.ResponseWriter, request *http.Request) {
	vars := hummingbird.GetVars(request)
	timestamp, err := hummingbird.StandardizeTimestamp(request.Header.Get("X-Timestamp"))
	if err != nil {
		hummingbird.StandardResponse(writer, http.StatusBadRequest)
		return
	}
	...
	if err == ErrorPolicyConflict {
		...
		hummingbird.GetLogger(request).LogError("Unable to create database: %v", err)
		hummingbird.StandardResponse(writer, http.StatusInternalServerError)
		return
	}
	defer server.containerEngine.Return(db)
	if info, err := db.GetInfo(); err == nil {
		server.accountUpdate(request, vars, info, hummingbird.GetLogger(request))
	}
	if created {
		hummingbird.StandardResponse(writer, http.StatusCreated)
	} else {
		hummingbird.StandardResponse(writer, http.StatusAccepted)
	}
}
...
func (server *ContainerServer) ContainerDeleteHandler(writer http.ResponseWriter, request *http.Request) {
	...
	if err = db.Delete(timestamp); err != nil {
		hummingbird.GetLogger(request).LogError("Unable to delete database: %v", err)
		hummingbird.StandardResponse(writer, http.StatusInternalServerError)
		return
	}
	info, err = db.GetInfo()
...
func (server *ContainerServer) ObjPutHandler(writer http.ResponseWriter, request *http.Request) {
	...
	if err == ErrorNoSuchContainer {
		if strings.HasPrefix(vars["account"], server.autoCreatePrefix) {
			if _, db, err = server.containerEngine.Create(vars, timestamp, map[string][]string{}, policyIndex, 0); err != nil {
				hummingbird.GetLogger(request).LogError("Unable to auto-create container: %v", err)
				hummingbird.StandardResponse(writer, http.StatusInternalServerError)
				return
			}
		} else {
	...
	if err := db.PutObject(vars["obj"], timestamp, size, contentType, etag, policyIndex); err != nil {
		hummingbird.GetLogger(request).LogError("Error adding object to container: %v", err)
		hummingbird.StandardResponse(writer, http.StatusInternalServerError)
		return
	}
	hummingbird.StandardResponse(writer, http.StatusCreated)
	...
	fn := func(writer http.ResponseWriter, request *http.Request) {
		...
		if device, ok := vars["device"]; ok && device != "" {
			...
			if server.checkMounts {
				if mounted, err := hummingbird.IsMount(devicePath); err != nil || !mounted {
...
func (server *ContainerServer) GetHandler(config hummingbird.Config) http.Handler {
...
func GetServer(serverconf hummingbird.Config, flags *flag.FlagSet) (bindIP string, bindPort int, serv hummingbird.Server, logger hummingbird.SysLogLike, err error) {
	...
	server.diskInUse = hummingbird.NewKeyedLimit(serverconf.GetLimit("app:container-server", "disk_limit", 25, 10000))
	...
	server.logger = hummingbird.SetupLogger(serverconf.GetDefault("app:container-server", "log_facility", "LOG_LOCAL0"), "container-server", "")
